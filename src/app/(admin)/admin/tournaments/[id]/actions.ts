...
async function recalculateRanking(tournamentId: string) {
  ...
  if (tournament.system === "americano" || tournament.system === "mexicano") {
...
// ── Unified round generation entry point ─────────────────
...
export async function generateNextRound(tournamentId: string) {
  ...
  if (tournament.system === "mexicano") {
    await generateNextMexicanoRound(tournamentId);
  } else {
    await generateNextAmericanoRound(tournamentId);
  }
}

// ── Americano round generation ────────────────────────────

async function getCourtNums(tournamentId: string, courts: number, storedNums: number[]): Promise<number[]> {
  return storedNums.length > 0 ? storedNums : Array.from({ length: courts }, (_, i) => i + 1);
}

export async function generateNextAmericanoRound(tournamentId: string) {
  const tournament = await prisma.tournament.findUnique({
    where: { id: tournamentId },
    select: { courts: true, pointsToWin: true, courtNumbers: true },
  });
  if (!tournament) return;

  const courts = tournament.courts;
  const courtNums = await getCourtNums(tournamentId, courts, tournament.courtNumbers);
  ...
  for (let i = 0; i < pairs.length; i += 2) {
    ...
    const courtIdx = i / 2;
    const courtNumber = courtNums[courtIdx] ?? courtIdx + 1;
...
// Pairs generated by ranking: rank1+rank2, rank3+rank4, ...

async function generateNextMexicanoRound(tournamentId: string) {
  const tournament = await prisma.tournament.findUnique({
    where: { id: tournamentId },
    select: { courts: true, pointsToWin: true, courtNumbers: true },
  });
  ...
  const individualTeams = await prisma.team.findMany({
    where: { tournamentId, player2Id: null },
    include: {
      rankingEntries: { where: { tournamentId } },
      player1: true,
    },
  });

  // Determine sit-out priority: players who played fewest rounds sit out last
  const pairTeams = await prisma.team.findMany({
    where: { tournamentId, player2Id: { not: null } },
  });
  const playedCount = new Map<string, number>();
  ...
  const activeCount = Math.floor(Math.min(courts * 4, prioritized.length) / 4) * 4;
  const activePrioritySet = new Set(prioritized.slice(0, activeCount));

  if (activeCount < 4) return;

  // Sort active players by ranking (points desc, wins tiebreak)
  const rankMap = new Map<string, { points: number; wins: number }>();
  for (const t of individualTeams) {
    rankMap.set(t.player1Id, {
      points: t.rankingEntries[0]?.points ?? 0,
      wins: t.rankingEntries[0]?.wins ?? 0,
    });
  }
  const activeSorted = [...activePrioritySet].sort((a, b) => {
    const ra = rankMap.get(a) ?? { points: 0, wins: 0 };
    const rb = rankMap.get(b) ?? { points: 0, wins: 0 };
    return rb.points - ra.points || rb.wins - ra.wins;
  });
  ...
  // Mexicano pairing: rank1+rank2 vs rank3+rank4, rank5+rank6 vs rank7+rank8, …
  for (let i = 0; i < activeSorted.length; i += 4) {
    const p1a = activeSorted[i];
    const p1b = activeSorted[i + 1];
    const p2a = activeSorted[i + 2];
    const p2b = activeSorted[i + 3];
    const courtIdx = i / 4;
    const courtNumber = courtNums[courtIdx] ?? courtIdx + 1;
  ...
  // Bye matches for sitting-out players
  const sittingOutIds = prioritized.slice(activeCount);
  const byePoints = Math.ceil(tournament.pointsToWin / 2);
  for (const playerId of sittingOutIds) {
    const indivTeam = individualTeams.find((t) => t.player1Id === playerId);
...
export async function generateAmericanoFinalRound(tournamentId: string) {
  const tournamentConfig = await prisma.tournament.findUnique({
    where: { id: tournamentId },
    select: { courts: true, courtNumbers: true },
  });
  const courtNums = tournamentConfig
    ? await getCourtNums(tournamentId, tournamentConfig.courts, tournamentConfig.courtNumbers)
    : [1, 2, 3, 4];
  ...
  for (let i = 0; i + 3 < sortedPlayers.length; i += 4) {
    ...
    await prisma.match.create({
      data: {
        ...
        team2Id: team2.id,
        round: finalRound,
        court: courtNums[courtIdx] ?? courtIdx + 1,
        group: "Final",
        status: "pending",
      },
    });
